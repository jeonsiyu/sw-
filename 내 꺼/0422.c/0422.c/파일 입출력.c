...
// r :: 읽기 모드
// w :: 쓰기 모드 , 처음부터 새로 작성(기존의 내용 삭제)
// w+ :: 일기 + 쓰기 모드, 파일이 있으면 새로운 데이터로 덮어씌워짐(w와 유사)
// a :: 추가 모드, 파일이 있으면 끝에 데이터 추가

int main (){
	// 파일 불러오기 :: fopen()
	//"sample.txt" 파일을 가리키는 포인터 변수 생성
	FILE* fp = fopen("sample.txt", "w+" ); // 특정 파일을 가리키는 포인터 변수 생성
	// NULL :: " 정보가 없는"
	if (fp != NULL) {		// fp 변수에 어떠한 정보라도 받았다면
		printf("파일 불러오기 성공\n");

		// 파일 입력 :: c파일 -> txt 파일 내용 작성
		// fputc(문자정보, FILE*) :: 문자 하나식 파일에 입력
		fputc('a', fp); 
		fputc('b', fp);
		fputc('c', fp);
		...
		char c;		// fp 파일 내용을 문자 단위로 하나씩 불러옴
		while (1) {
	...
	else {			// "fp == NULL" => fp 변수에 어떠한 정보도 없다면
		printf("파일 불러오기 실패\n");
